  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    if (type === 4) {
      ...
      if (Array.isArray(value)) {
        fixed[key] = value.length > 0 ? String(value[0]) : '';
      } else {
        fixed[key] = value != null ? String(value) : '';
      }
    } else if (type === 5) {
      // DateTime: 必须是Unix时间戳(毫秒)
      if (typeof value === 'string') {
        fixed[key] = new Date(value).getTime() || Date.now();
...
async function getToken() {
  ...
  if (data.code === 0) {
    tokenCache = { token: data.tenant_access_token, expireAt: Date.now() + (data.expire - 60) * 1000 };
    return tokenCache.token;
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { action, table_id, record_id } = req.query;

    if (!action || !table_id) {
      return res.status(400).json({ error: 'Missing action or table_id' });
    }

    if (action === 'list') {
      const pageSize = req.query.page_size || 100;
      const filter = req.query.filter || '';
      let url = `https://open.feishu.cn/open-apis/bitable/v1/apps/${FEISHU_BASE_TOKEN}/tables/${table_id}/records?page_size=${pageSize}`;
    ...
    if (action === 'create') {
      ...
      if (body.records && Array.isArray(body.records)) {
        body.records = body.records.map(r => ({
          ...r,
          fields: fixFields(table_id, r.fields || {})
        }));
      }
    ...
    if (action === 'update') {
      ...
      const body = req.body || {};
      if (body.fields) {
        body.fields = fixFields(table_id, body.fields);
      }
